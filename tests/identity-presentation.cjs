...
assert.match(html,/club-visual-empty/);assert.match(html,/--club-secondary:#FFFFFF/);assert.equal((html.match(/<svg /g)||[]).length,3);assert(!html.includes('club-stadium-cover'));assert.equal(JSON.stringify(ctx.c),before);
