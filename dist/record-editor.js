...
function clubVisual(c,detail=false){
 ...
 return `<div class="club-visual ${detail?'club-visual-large':''} ${c.stadiumPhoto?'':'club-visual-empty'}" style="${cardIdentityStyle(c)}">${cardIdentityArtwork(c,{stadium:true,lazy:!detail})}${clubLowerWave()}<div class="club-visual-identity">${badge(c)}<div><span class="eyebrow">${esc(divisionLabel(c))}</span><${detail?'h2':'h3'}>${clubNameMarkup(c)}</${detail?'h2':'h3'}><div class="${detail?'club-detail-meta':'club-card-meta'}">${meta}</div></div></div></div>`;
}
